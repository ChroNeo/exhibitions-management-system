...
const ExhibitionForm = forwardRef<HTMLFormElement, Props>(
  function ExhibitionForm(
    ...
    const [form, setForm] = useState<ExhibitionFormValues>({
      ...EMPTY,
      ...initialValues,
    });
    ...
    const update = <K extends keyof ExhibitionFormValues>(
      k: K,
    ...
    useEffect(() => {
      ...
      const normStart =
        form.start_date || isoToLocalInput(initialValues?.start_date);
      ...
      if (
        ...
        const clip = quill.clipboard.convert({
          html: initialValues.description,
        });
    ...
    useEffect(() => {
      ...
      try {
        ...
        const sameScope =
          draft.version === 1 && draft.exhibitionId === exhibitionId;
        if (!sameScope) return;
    ...
    const handleDetailPdfRemove = useCallback(() => {
      setForm((prev) => ({
        ...prev,
        detailPdfFile: undefined,
        detailPdfRemoved: prev.detailPdfFile ? false : !!initialDetailPdfName,
      }));
    ...
    const statusOptions = useMemo(() => {
      ...
      if (form.status && !values.includes(form.status))
